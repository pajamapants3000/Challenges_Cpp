...
which is represented by a 2. You are able to wrap around one side of the matrix to the other as
well. For example: if strArr is ["0000", "1000", "0002", "0002"] then this looks like the following:
...
For this input your program should return 2 because the closest enemy (2) is 2 spaces away from the
1 by moving left to wrap to the other side and then moving down once. The array will contain any
number of 0's and 2's, but only a single 1. It may not contain any 2's at all as well, where in
...
*/
#ifndef CLOSESTENEMY_H
#define CLOSESTENEMY_H
...
class ClosestEnemy : public Solution
{
public:
    ClosestEnemy();
...
private:
    ...
    bool isEnemyOnDiagonal(size_t distance, Corner corner) const;
    void findSelf();

    std::vector<size_t> selfPos;
};

#endif // CLOSESTENEMY_H
